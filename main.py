...
def train_test_fn(model, train_loader, val_loader, save_file_format=None):
	...
	metric = metric_fn.evaluate(predict=scores, label=labels, is_final=False)
...
def train_fn(config, model, train_loader, val_loader):
	...
	# callback.append(pl.callbacks.StochasticWeightAveraging())
...
def train(config, model_cls=FUSIONNET):
	...
	# 	tmp = metric_fn.evaluate(predict=scores[:i], label=labels[:i])
	# 	k_final[i] = tmp
	# for k, v in k_final.items():
	# 	print("k_final_{} result:{}".format(k, v))
	final_metric = metric_fn.evaluate(predict=scores, label=labels)
...
def parse(print_help=False):
	...
	parser = FUSIONNET.add_model_specific_args(parser)
